...
def get_model_path_from_resolution(onnx_models_path, width, height,name):
    onnx_model_path = os.path.join(onnx_models_path, f"sam_onnx.{height}_{width}_{name}.onnx")
    return onnx_model_path

class OnnxModels:
    def __init__(self, onnx_models_path, threshold=0.5, image_width=1920, image_height=1080,name="original1"):
        self.onnx_models_path = onnx_models_path
        print(self.onnx_models_path)
        self.threshold = threshold
        self.set_image_resolution(image_width, image_height, name)
    ...
    def set_image_resolution(self, width, height,name):
        self.image_width = width
        self.image_height = height
        onnx_model_path = get_model_path_from_resolution(
            self.onnx_models_path, width, height,name
        )
